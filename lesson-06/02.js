...
Ваша функция `map` должна возвращать новый массив, где каждый элемент является
результатом вызова функции `callback` на каждом элементе исходного массива.
*/
...
const map = (array, callback) => {
  ...
  for (let i = 0; i < array.length; i++) {
    mapArray.push(callback(array[i], i))
  }

  return mapArray
}

const doubledNumbers = (element, index) => {
  return element * 2
}

console.log(map([1, 2, 3, 4, 5], doubledNumbers))
